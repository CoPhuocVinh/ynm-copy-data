...
export class RabbitMQWrapperService implements OnModuleInit {
  ...
  async initialize() {
    ...
    try {
      ...
      // Setup queues
      const queueName = this.configService.get<string>('queue.comments.name');
      const durable = this.configService.get<boolean>('queue.comments.durable');
      const autoDelete = this.configService.get<boolean>('queue.comments.autoDelete');
      ...
      this.logger.log(`Queue ${queueName} asserted successfully`);
